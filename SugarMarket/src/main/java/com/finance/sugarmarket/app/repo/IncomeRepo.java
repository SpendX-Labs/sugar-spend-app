...
public interface IncomeRepo extends JpaRepository<Income, Long>, JpaSpecificationExecutor<Income> {
	...
	@Query("SELECT COALESCE(SUM(i.amount), 0) FROM Income i WHERE i.user.id = :userId AND YEAR(i.dateOfEvent) = :year AND MONTHNAME(i.dateOfEvent) = :month")
	BigDecimal getTotalMonthlyIncome(Integer year, String month, Long userId);

	@Query("SELECT COALESCE(SUM(i.amount), 0) FROM Income i WHERE i.user.id = :userId AND YEAR(i.dateOfEvent) = :year")
