...
public class BudgetViewerService {
	...
	public ExpenseReportDto getExpenseReport(Long userId, String month, Integer year) {
		...
		reportDto.setTotalAmount(manualSpendAmount.add(reportDto.getAutoDebitAmount()));
	...
	private void setAutoDebit(List<BudgetView> budgetList, ExpenseReportDto reportDto, Date today) {
		...
		if (dueDateIndex < 0) {
			...
			reportDto.setAutoDebitAmount(budgetList.subList(0, dueDateIndex).stream().map(BudgetView::getActualAmount)
					.reduce(BigDecimal.ZERO, BigDecimal::add));
			reportDto.setRemainingAutoDebit(getAutoDebit(budgetList.subList(dueDateIndex, budgetList.size())));
		} else {
